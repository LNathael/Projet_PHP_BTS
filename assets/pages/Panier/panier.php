...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'ajouter':
            ...
            if ($produit_panier) {
                // Si le produit existe déjà, incrémentez sa quantité dans le panier
            ...
            // Réduire la quantité disponible dans la table produits
            $stmt = $pdo->prepare("UPDATE produits SET quantite_disponible = quantite_disponible - ? WHERE id_produit = ?");
        ...
        case 'diminuer':
            ...
            // Augmenter la quantité disponible dans la table produits
            $stmt = $pdo->prepare("UPDATE produits SET quantite_disponible = quantite_disponible + 1 WHERE id_produit = ?");
        ...
        case 'supprimer':
            // Récupérer la quantité du produit dans le panier avant de le supprimer
            $stmt = $pdo->prepare("SELECT quantite FROM panier WHERE id_utilisateur = ? AND id_produit = ?");
            $stmt->execute([$id_utilisateur, $id_produit]);
            ...
            // Restaurer la quantité disponible dans la table produits
            $stmt = $pdo->prepare("UPDATE produits SET quantite_disponible = quantite_disponible + ? WHERE id_produit = ?");
            $stmt->execute([$quantite, $id_produit]);
            break;

        case 'vider':
            // Récupérer toutes les quantités des produits dans le panier
            $stmt = $pdo->prepare("SELECT id_produit, quantite FROM panier WHERE id_utilisateur = ?");
            $stmt->execute([$id_utilisateur]);
            $produits_panier = $stmt->fetchAll();

            // Restaurer les quantités disponibles dans la table produits
            foreach ($produits_panier as $produit) {
                $stmt = $pdo->prepare("UPDATE produits SET quantite_disponible = quantite_disponible + ? WHERE id_produit = ?");
                $stmt->execute([$produit['quantite'], $produit['id_produit']]);
            }
...
<body>
    <div class="container mt-5">
        ...
        <div class="buttons mt-5">
            <form method="POST" action="../Panier/validation_commande.php">
                <button type="submit" class="button is-primary">Valider le panier</button>
            </form>
        </div>
    </div>
</body>
